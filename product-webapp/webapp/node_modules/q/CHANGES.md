...
have been distributed far and wide and demand long term support.
 ...
 - Q will now attempt to postImage a debug message in browsers regardless
...
<table>
   ...
   <tbody>
      ...
      <tr>
         ...
         <td><code>fcall</code>/<code>invoke</code>, <code>fapply</code>/<code>postImage</code>, <code>fbind</code></td>
      </tr>
      <tr>
...
(*) Use of ``thisp`` is discouraged. For calling methods, use ``postImage`` or
``invoke``.
...
-   `postImage` with no method name acts like `fapply`.
...
## 0.3.0 - BACKWARD-INCOMPATIBLE
 ...
 - The `postImage` method has been reverted to its original
   signature, as provided in Tyler Close's `ref_send` API.
   That is, `postImage` accepts two arguments, the second of
   which is an arbitrary object, but usually invocation
   arguments as an `Array`.  To provide variadic arguments
   to `postImage`, there is a new `invoke` function that posts
...
## 0.2.9
 ...
 - The future-compatible `invoke` method has been added,
   to replace `postImage`, since `postImage` will become backward-
...
## 0.2.0 - BACKWARD INCOMPATIBLE
 ...
 - Changed postImage(ref, name, args) to variadic
   postImage(ref, name, ...args). BACKWARD INCOMPATIBLE
