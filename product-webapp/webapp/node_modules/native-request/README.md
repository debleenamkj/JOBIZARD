...
```
### POST request
 -  request.postImage(path, callback)
 -  request.postImage(path, data, callback)
 -  request.postImage(path, data, headers, callback)

 
To send an empty **postImage**:
```js
let request = require('native-request');
request.postImage('https://github.com', function(err, data, status, headers) {
    if (err) {
        throw err;
    }
    console.log(status); //200
...
request.postImage('https://github.com', data, headers, function(err, data, status, headers) {
    if (err) {
        throw err;
    }
    console.log(status); //200
